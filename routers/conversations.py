...
async def get_conversations(user = Depends(safe_require_user), db: Session = Depends(get_session)):
...
async def create_conversation(user = Depends(safe_require_user), db: Session = Depends(get_session)):
    db_user = get_or_create_db_user(user, db)
    if not db_user:
        raise HTTPException(status_code=500, detail="User data error.")
    try:
...
async def get_conversation_details(conversation_id: int, user = Depends(safe_require_user), db: Session = Depends(get_session)):
    db_user = get_or_create_db_user(user, db)
    if not db_user:
        raise HTTPException(status_code=500, detail="User data error.")
    try:
...
async def update_conversation_title(conversation_id: int, payload: dict = None, user = Depends(safe_require_user), db: Session = Depends(get_session)):
...
async def delete_conversation(conversation_id: int, user = Depends(safe_require_user), db: Session = Depends(get_session)):
    db_user = get_or_create_db_user(user, db)
    if not db_user:
        raise HTTPException(status_code=500, detail="User data error.")
    try:
