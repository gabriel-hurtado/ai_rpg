...
const ChatManager = {
  ...
  async handleAPISubmission(userMessage, aiMessageDiv, textareaElement) {
    console.log('[Chat][handleAPISubmission] Function called. User message:', userMessage); // <-- Add this log
    const token = window.getCurrentAccessToken ? window.getCurrentAccessToken() : null;
    let success = false;
    ...
    if (!token) {
      console.error('[Chat][handleAPISubmission] No token found.'); // <-- Add log
      if (aiMessageDiv) {
    ...
    try {
      console.log('[Chat][handleAPISubmission] Attempting to send message to API. Conversation ID:', this.currentConversationId); // <-- Add this log
      const response = await sendChatMessage({ prompt: userMessage, conversationId: this.currentConversationId, token });
      console.log('[Chat][handleAPISubmission] Received response from API. Status:', response.status); // <-- Add this log

      if (!response.ok) {
          const errorText = await response.text();
          console.error('[Chat][handleAPISubmission] API response not OK:', response.status, errorText); // <-- Add log
      ...
      if (!markdownDiv) {
          console.error('[Chat][handleAPISubmission] Markdown display area not found in aiMessageDiv.'); // <-- Add log
          throw new Error('Markdown display area not found.');
      }

      console.log('[Chat][handleAPISubmission] Starting stream reading loop...'); // <-- Add this log
      while (true) {
        const { done, value } = await reader.read();
        if (done) {
            console.log('[Chat][handleAPISubmission] Stream finished (done).'); // <-- Add this log
            break;
        }
        let chunk = decoder.decode(value, { stream: true });
        console.log('[Chat][handleAPISubmission] Received raw chunk:', JSON.stringify(chunk)); // Log raw chunk

        let jsonPayload = null;
        let textContent = chunk; // Assume it's all text initially

        // --- MODIFICATION START: Check if chunk contains the JSON payload ---
        const potentialJsonStart = chunk.lastIndexOf('{"userMessageId":');
        // Check if we found the start marker AND the trimmed chunk ends with '}'
        if (potentialJsonStart !== -1 && chunk.trim().endsWith('}')) {
            ...
            try {
                ...
                if (parsed.userMessageId && parsed.aiMessageId) {
                    ...
                    console.log('[Chat][handleAPISubmission] Extracted JSON payload:', jsonPayload);
                    console.log('[Chat][handleAPISubmission] Remaining text content:', JSON.stringify(textContent));
                } else {
                     console.log('[Chat][handleAPISubmission] Parsed object from chunk end, but missing required keys.');
                     // Treat as text if keys are wrong
                     jsonPayload = null;
                     textContent = chunk;
                }
            } catch (e) {
                // Failed to parse, treat the whole chunk as text
                 console.log('[Chat][handleAPISubmission] Found potential JSON structure at end, but failed to parse:', e.message);
                jsonPayload = null;
                textContent = chunk; // Reset to full chunk if parsing failed
            }
        } else {
             // Log if the chunk doesn't seem to contain the final JSON
             // console.log('[Chat][handleAPISubmission] Chunk does not appear to contain final JSON payload.');
        }
        // --- MODIFICATION END ---
        ...
        if (jsonPayload) {
            console.log('[Chat] Processing final message IDs:', jsonPayload);

            // --- Assign IDs ---
            aiMessageDiv.dataset.messageId = jsonPayload.aiMessageId;
            if (userMessageElement) {
                 userMessageElement.dataset.messageId = jsonPayload.userMessageId;
                 console.log(`[Chat] Set user message element ID (via previousElementSibling) to ${jsonPayload.userMessageId}`);
            } else {
                 console.warn(`[Chat] Could not find user message element (via previousElementSibling) to set ID ${jsonPayload.userMessageId}`);
            }
            // --- End Assign IDs ---
            ...
            const chatDisplay = document.getElementById('chat-message-list');
            if (!chatDisplay) {
                console.error("[Chat] Cannot add delete buttons: chat-message-list not found.");
            } else {
                const messages = chatDisplay.children;
                // Recalculate indices just before adding buttons
                const aiMessageIndex = messages.length - 1;
                const userMessageIndex = messages.length - 2;

                // Target AI message (which is aiMessageDiv)
                console.log(`[Chat] Attempting to add delete button to AI message at index ${aiMessageIndex}`);
                this.addDeleteButtonAndListener(aiMessageDiv, aiMessageIndex); // Use the direct reference

                // Target User message using index
                if (userMessageIndex >= 0) {
                    const userMessageDivAtIndex = messages[userMessageIndex];
                    if (userMessageDivAtIndex && userMessageDivAtIndex.classList.contains('user-message')) { // Added class check
                        // Ensure the ID is set (it should have been by the code above)
                        if (userMessageDivAtIndex.dataset.messageId == jsonPayload.userMessageId) {
                             console.log(`[Chat] Attempting to add delete button to User message at index ${userMessageIndex}`);
                             this.addDeleteButtonAndListener(userMessageDivAtIndex, userMessageIndex);
                        } else {
                             console.warn(`[Chat] User message element at index ${userMessageIndex} found, but ID (${userMessageDivAtIndex.dataset.messageId}) doesn't match expected (${jsonPayload.userMessageId}). Forcing ID and adding button.`);
                             // Force set ID and add button anyway
                             userMessageDivAtIndex.dataset.messageId = jsonPayload.userMessageId;
                             this.addDeleteButtonAndListener(userMessageDivAtIndex, userMessageIndex);
                        }
                    } else {
                         console.warn(`[Chat] Could not find valid user message element at index ${userMessageIndex} to add delete button.`);
                    }
                } else {
                     console.warn(`[Chat] Invalid user message index calculated (${userMessageIndex}). Cannot add delete button.`);
                }
            }
            // --- End Add Delete Buttons ---

            // IMPORTANT: Break the loop after processing the JSON payload
            // as it signifies the end of the response stream.
            break;
        }
      } // End while loop
  ...
  async deleteMessageAndAfter(messageIndex) {
    ...
    if (!messageIdToDelete) {
      // Fallback or error if ID is missing, though it should be set now
    ...
    console.log(`[Chat] Attempting to delete message ID ${messageIdToDelete} (index ${messageIndex}) and after in conversation ${conversationId}`);
    ...
    try {
        ...
        console.log(`[Chat] Successfully deleted message ID ${messageIdToDelete} via API.`);

        // --- Remove messages from UI ---
        // Remove elements from the clicked index onwards
        const messagesToRemove = Array.from(chatDisplay.children).slice(messageIndex);
        messagesToRemove.forEach(msg => msg.remove());
        console.log(`[Chat] Removed ${messagesToRemove.length} message elements from UI starting at index ${messageIndex}.`);

        // Optional: Reload conversation to ensure perfect sync, though UI removal might suffice
        // await this.loadAndDisplayConversation(conversationId);
